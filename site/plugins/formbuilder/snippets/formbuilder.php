...
<div<?php if($pg->fb_formbtns_class()->isNotEmpty()): ?> class="<?= $pg->fb_formbtns_class()->html() ?>"<?php endif; ?>>
    <button class="transition ease-in-out duration-500 bg-crimson hover:bg-crimson-dark transform hover:scale-105 text-white px-6 py-3 rounded-md uppercase font-bold tracking-wider mt-5" type="submit" name="submit"><?= $pg->fb_submit_label()->or("Submit")->html() ?> <i class="fas fa-paper-plane"></i></button>
